...
/**
 * A custom view designed to manage and display a [Painter] and provide touch gestures and layer management capabilities.

 * **Differences with [LayeredPainterView]:**
 * - This class doesn't support multiple layers; it operates on a single layer.
 * - This class doesn't support undo/redo functionality and does not save its state (will be added in future updates.)
 *
 * **Core Features:**
 ...
 * - **Painter Integration**: Manages [Painter] objects.
 * - **Touch Mapping**: Precise coordinate transformation for accurate painting.
 * - **Performance Optimization**: Gesture detection and rendering optimization.
 *
 * **Gesture System:**
 ...
 * - **Gesture Delegation**: Optional gesture forwarding to users.
...
open class PainterView(context: Context, attrSet: AttributeSet?) :
    ...
    protected val matrixAnimator by lazy {
        MatrixAnimator(canvasMatrix, RectF(boundsRectangle), 300L, FastOutSlowInInterpolator())
    }
     ...
     */
    protected val imageviewMatrix = SimurghMatrix()
    ...
    protected val endMatrix = SimurghMatrix()
    protected val startMatrix = SimurghMatrix()
    ...
    protected open val resetMatrixAnimator by lazy {
        ValueAnimator.ofObject(MatrixEvaluator(), startMatrix, endMatrix).apply {
            ...
            addUpdateListener {
                canvasMatrix.set(it.animatedValue as SimurghMatrix)
                invalidate()
            }
        }
    }
